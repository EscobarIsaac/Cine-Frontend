...
export default function Entradas() {
  ...
  async function load(){ setError(''); try{ const { data }=await api.get('/entradas'); setItems(Array.isArray(data)?data:[]) } catch(err){ setError(asMessage(err)) } }
  useEffect(()=>{ load() }, [])

  const fields = useMemo(()=> items.length? Object.keys(items[0]).filter(k=>k.toLowerCase()!=='id') : [], [items])
  ...
  async function crear(e){
    ...
    try{
      let payload
      if(jsonMode){ payload = JSON.parse(jsonBody||'{}') }
      else{
        payload = {}
  ...
  return (
    <div>
      <h1>Entradas <span className="badge">{items.length}</span></h1>
