...
export default function Eventos() {
  ...
  async function load(){ setError(''); try{ const { data }=await api.get('/eventos'); setItems(Array.isArray(data)?data:[]) } catch(err){ setError(asMessage(err)) } }
  useEffect(()=>{ load() }, [])
  ...
  return (
    <div>
      <h1>Eventos <span className="badge">{items.length}</span></h1>
